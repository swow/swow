...
Coroutine::run(function () use ($server) {
    ...
    while (true) {
        $packet = $server->recvStringFrom(MAX_LENGTH_LOW, $ip, $port);
        if (!$packet) {
            break;
        }
...
$randoms = getRandomBytesArray(MAX_REQUESTS, MAX_LENGTH_LOW);
for ($n = 0; $n < MAX_REQUESTS; $n++) {
    $client->sendStringTo($randoms[$n], $server->getSockAddress(), $server->getSockPort());
}
for ($n = 0; $n < MAX_REQUESTS; $n++) {
    $packet = $client->recvStringFrom(MAX_LENGTH_LOW, $ip, $port);
