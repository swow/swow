...
class WindowsConstantDefinitionsFetcher extends ConstantDefinitionsFetcherAbstract
{
    ...
    const SIGNAL_RE = '/#define\s+(?<name>SIG[A-Z]+)\s+(?<value>\d+)\s*\/\/\s*(?<comment>.+)/';
    const SIGNAL_HEADER_URL = '/generation/WinSDK/RecompiledIdlHeaders/ucrt/signal.h';

    const ERRNO_RE = '/#define\s+(?<name>E[A-Z0-9]+)\s+(?<value>\d+)/';
    const ERRNO_HEADER_URL = '/generation/WinSDK/RecompiledIdlHeaders/ucrt/errno.h';
