...
SWOW_API CAT_COLD void swow_exception_set_properties(zend_object *exception, const char *message, zend_long code)
{
    ...
    if (message != NULL) {
        ...
        zend_update_property_ex(exception->ce, &zexception, ZSTR_KNOWN(ZEND_STR_MESSAGE), &ztmp);
    }
    if (code != (zend_long) ~0) {
        ZVAL_LONG(&ztmp, code);
        zend_update_property_ex(exception->ce, &zexception, ZSTR_KNOWN(ZEND_STR_CODE), &ztmp);
    }
}
...
SWOW_API CAT_COLD void swow_call_exception_set_return_value(zend_object *exception, zval *return_value)
{
    ...
    ZVAL_OBJ(&zexception, exception);
    zend_update_property(exception->ce, &zexception, ZEND_STRL("returnValue"), return_value);
}
