...
typedef struct swow_php_token_s {
    cat_queue_t node;
    int type;
...
typedef struct swow_php_token_list_s {
    cat_queue_t queue;
    size_t count;
    zend_string *source;
} swow_php_token_list_t;

typedef enum swow_php_ast_walker_op_e {
    SWOW_PHP_AST_WALKER_CONTINUE = 0, /* continue walk */
    SWOW_PHP_AST_WALKER_STOP = 1, /* stop walk */
    SWOW_PHP_AST_WALKER_SKIP = 2, /* skip children walk */
} swow_php_ast_walker_op;

typedef swow_php_ast_walker_op (*swow_php_ast_walker_callback_t)(zend_ast *ast, void *context);

typedef struct swow_php_ast_walker_s {
    swow_php_ast_walker_callback_t walker;
    void *context;
} swow_php_ast_walker_t;

SWOW_API void swow_php_ast_walk(zend_ast *ast, void *context);

typedef void (*swow_php_ast_callback_t)(zend_ast *ast, void *context);
