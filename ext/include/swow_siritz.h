...
typedef struct swow_siritz_run_s {
    /* data for thread startup */
    const unsigned char *callable;
    size_t callable_len;
    const unsigned char *args;
    size_t args_len;
    void *server_context;
    /* data for thread recycling */
    uv_sem_t sem; // semaphore for starting and finishing thread
    uv_mutex_t mutex; // lock for status
    int status;
} swow_siritz_run_t;
